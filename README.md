# Timeline - 语音驱动的日程记录工具

Timeline 是一个语音驱动的日程记录工具，通过语音输入自动分析并创建日历条目。主要用于记录用户每天做的事情，通过语音快速记录时间线和活动。

## 功能特性

- 🎙️ **语音转录**：支持中英文混合语音识别
- ⏰ **时间提取**：智能识别时间段和相对时间
- 📅 **事件提取**：自动提取活动、地点等信息
- 🌍 **时区支持**：自动处理时区转换
- 🚀 **快速部署**：支持一键部署到 ai-builders.space

## 项目结构

```
timeline/
├── app/
│   ├── __init__.py
│   ├── main.py              # FastAPI 应用入口
│   ├── api/
│   │   ├── __init__.py
│   │   └── routes.py        # API 路由
│   ├── services/
│   │   ├── __init__.py
│   │   ├── transcription.py # 音频转录服务
│   │   └── llm_extractor.py # LLM 事件提取服务
│   ├── models/
│   │   ├── __init__.py
│   │   ├── request.py       # 请求模型
│   │   └── response.py      # 响应模型
│   └── utils/
│       ├── __init__.py
│       ├── timezone.py      # 时区处理工具
│       └── prompt_loader.py # Prompt 模板加载器
├── prompts/                 # LLM Prompt 模板
│   └── prompts.md          # Prompt 模板文件
├── static/                  # 静态文件（测试页面）
│   └── index.html          # 测试页面
├── tests/                   # 测试文件
│   ├── __init__.py
│   └── test_api.py
├── Dockerfile              # Docker 配置
├── requirements.txt        # Python 依赖
└── README.md
```

## 快速开始

### 本地开发（推荐使用 uv）

```bash
# 1. 安装 uv（如果还没有）
curl -LsSf https://astral.sh/uv/install.sh | sh
# 或
pip install uv

# 2. 初始化项目（如果还没有）
uv venv

# 3. 激活虚拟环境
source .venv/bin/activate  # Linux/Mac
# 或
.venv\Scripts\activate  # Windows

# 4. 安装依赖
uv pip install -r requirements.txt

# 5. 运行服务（开发模式，支持热重载）
uv run uvicorn app.main:app --reload --host 0.0.0.0 --port 8000
```

### 使用传统方式

```bash
# 1. 创建虚拟环境
python -m venv venv

# 2. 激活虚拟环境
source venv/bin/activate  # Linux/Mac
# 或
venv\Scripts\activate  # Windows

# 3. 安装依赖
pip install -r requirements.txt

# 4. 运行服务
uvicorn app.main:app --reload --host 0.0.0.0 --port 8000
```

### 环境变量设置

创建 `.env` 文件（不要提交到 Git）：

```bash
AI_BUILDER_TOKEN=your_token_here
```

或直接设置环境变量：

```bash
export AI_BUILDER_TOKEN=your_token_here
```

## API 文档

### POST /api/v1/analyze

分析音频文件，提取时间和事件信息。

**请求格式**：
- Content-Type: `multipart/form-data`
- 参数：
  - `audio`: 音频文件（必需）
  - `timezone`: 时区字符串，如 "Asia/Shanghai"（必需）
  - `current_time`: ISO 8601 格式的当前时间（必需）

**响应示例**（成功，HTTP 200）：

```json
{
  "success": true,
  "transcription": "我2点去买菜，3点去理发，4点到家",
  "events": [
    {
      "title": "买菜",
      "start_time": "2024-01-15T14:00:00+08:00",
      "end_time": "2024-01-15T15:00:00+08:00",
      "description": "地点：超市"
    },
    {
      "title": "理发",
      "start_time": "2024-01-15T15:00:00+08:00",
      "end_time": "2024-01-15T15:30:00+08:00",
      "description": "地点：理发店"
    }
  ]
}
```

**响应示例**（错误，HTTP 4xx/5xx）：

```json
{
  "error": {
    "code": "TRANSCRIPTION_FAILED",
    "message": "语音识别失败，请重试",
    "details": "无法识别音频格式"
  }
}
```

### GET /health

健康检查端点。

**响应**：

```json
{
  "status": "healthy"
}
```

### GET /

返回 API 信息。

## 技术栈

- **FastAPI**: 现代、快速的 Web 框架
- **Uvicorn**: ASGI 服务器
- **httpx**: 异步 HTTP 客户端
- **pytz**: 时区处理
- **python-dateutil**: 时间解析
- **Pydantic**: 数据验证

## 部署

### 部署到 ai-builders.space

1. 确保代码已推送到 GitHub 公开仓库
2. 确保 Dockerfile 和 requirements.txt 已创建
3. 调用部署 API（参考 `docs/deployment-prompt.md`）

### Docker 构建

```bash
docker build -t timeline .
docker run -p 8000:8000 -e AI_BUILDER_TOKEN=your_token timeline
```

## 使用场景

### Web 应用

Timeline 主要作为 Web 应用使用：

1. 在浏览器中录音
2. 上传音频文件到 Timeline API
3. 接收 JSON 格式的事件列表
4. 在网页中查看和导出到日历（Apple Calendar 或 Google Calendar）

**Web 工作流程**：

```
1. 访问 Web 页面
   ↓
2. 点击录音按钮开始录音
   ↓
3. 停止录音，自动上传音频文件
   ↓
4. 接收分析结果（转录文本和事件列表）
   ↓
5. 查看事件列表
   ↓
6. 导出到日历（Apple Calendar 或 Google Calendar）
```

## 开发说明

### LLM 事件提取

本项目使用 LLM（通过 AI Builder Space API）进行时间解析和事件提取，而不是传统的 NLP 规则。这提供了更好的灵活性和准确性。

**使用的模型**：`gemini-3-flash-preview`（快速推理，适合时间解析任务）

**Prompt 设计**：参考 [TimeFlow](https://github.com/lindapu-1/TimeFlow) 项目的设计理念。

### 时区处理

- 支持时区名称（如 "Asia/Shanghai"）
- 支持时区偏移量（如 "+08:00"）
- 自动处理时区转换

### 错误处理

- HTTP 400：请求格式错误
- HTTP 422：处理失败（无法识别时间、无法提取事件）
- HTTP 500：服务器错误

## 许可证

MIT License

## 参考资源

- [TimeFlow 项目](https://github.com/lindapu-1/TimeFlow)：参考其 Prompt 设计和实现思路
- [TimeFlow prompts.md](https://github.com/lindapu-1/TimeFlow/blob/main/prompts.md)：详细的 Prompt 模板设计
